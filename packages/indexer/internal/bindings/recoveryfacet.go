...
// RecoveryFacetTransferSingleIterator is returned from FilterTransferSingle and is used to iterate over the raw logs and unpacked data for TransferSingle events raised by the RecoveryFacet contract.
type RecoveryFacetTransferSingleIterator struct {
	Event *RecoveryFacetTransferSingle // Event containing the contract specifics and raw log
...
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *RecoveryFacetTransferSingleIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	...
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(RecoveryFacetTransferSingle)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
...
// Error returns any retrieval or parsing error occurred during filtering.
func (it *RecoveryFacetTransferSingleIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *RecoveryFacetTransferSingleIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// RecoveryFacetTransferSingle represents a TransferSingle event raised by the RecoveryFacet contract.
type RecoveryFacetTransferSingle struct {
...
func (_RecoveryFacet *RecoveryFacetFilterer) FilterTransferSingle(opts *bind.FilterOpts, operator []common.Address, from []common.Address, to []common.Address) (*RecoveryFacetTransferSingleIterator, error) {
	...
	logs, sub, err := _RecoveryFacet.contract.FilterLogs(opts, "TransferSingle", operatorRule, fromRule, toRule)
	if err != nil {
		return nil, err
	}
	return &RecoveryFacetTransferSingleIterator{contract: _RecoveryFacet.contract, event: "TransferSingle", logs: logs, sub: sub}, nil
}

// WatchTransferSingle is a free log subscription operation binding the contract event 0xc3d58168c5ae7397731d063d5bbf3d657854427343f4c083240f7aacaa2d0f62.
//
// Solidity: event TransferSingle(address indexed operator, address indexed from, address indexed to, uint256 id, uint256 value)
func (_RecoveryFacet *RecoveryFacetFilterer) WatchTransferSingle(opts *bind.WatchOpts, sink chan<- *RecoveryFacetTransferSingle, operator []common.Address, from []common.Address, to []common.Address) (event.Subscription, error) {
	...
	logs, sub, err := _RecoveryFacet.contract.WatchLogs(opts, "TransferSingle", operatorRule, fromRule, toRule)
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(RecoveryFacetTransferSingle)
				if err := _RecoveryFacet.contract.UnpackLog(event, "TransferSingle", log); err != nil {
					return err
				}
				event.Raw = log
...
//
// Solidity: event TransferSingle(address indexed operator, address indexed from, address indexed to, uint256 id, uint256 value)
func (_RecoveryFacet *RecoveryFacetFilterer) ParseTransferSingle(log types.Log) (*RecoveryFacetTransferSingle, error) {
	event := new(RecoveryFacetTransferSingle)
	if err := _RecoveryFacet.contract.UnpackLog(event, "TransferSingle", log); err != nil {
		return nil, err
	}
