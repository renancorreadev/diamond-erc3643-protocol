...
export function renderGasReport(data: GasReportData, outputDir: string): string {
    ...
    .map(
      ...
      <div class="flex items-center justify-between text-sm py-1 border-b border-dark-700/50">
        ...
        <div class="flex items-center gap-4">
          <span class="text-gray-500">${d.previousAvg} → ${d.currentAvg}</span>
  ...
  const content = `<div x-cloak>${alertBanner}${contractTables}${diffTable}</div>`;
  ...
  const outputPath = join(outputDir, "gas-report.html");
