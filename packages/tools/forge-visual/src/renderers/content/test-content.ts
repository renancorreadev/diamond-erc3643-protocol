...
export function renderTestContent(report: TestReport): string {
...
<div x-data="{
  ...
  sortAsc: true,
  get filteredSuites() {
    return window.__testSuites.filter(s => {
...
export function renderTestScript(report: TestReport): string {
  ...
  return `<script>
window.__testSuites = ${suitesJson};
(function() {
  const suites = window.__testSuites;
